...
const errResponse = require('../utility/response');
...
const jwtMiddleware = (req, res, next) => {
    ...
    const p = new Promise(
        (resolve, reject) => {
            jwt.verify(token, process.env.JWT_SECRET, (error, verifiedToken) => {
                if (error) {
                    console.log(`jwtMiddlwareError: ${error}`);
                    reject(error);
                }
    ...
    p.then(async verifiedToken => {
        ...
        const loginCheckbyToken = await this.UserService.checkValidAccess(verifiedToken.userIdx);
